...
function export_td( $args = array() ) {
	...
	$defaults = array( 'content' => 'all', 'author' => false, 'category' => false,
		'start_date' => false, 'end_date' => false, 'status' => false, 'filename' => $filename, 'ids' => array()
	);
...
<?php if ( $post_ids ) {
	...
	$where = 'WHERE ID IN (' . join( ',', $next_posts ) . ') ORDER BY FIELD(ID, ' . join( ',', $next_posts ) . ' )';
...
<?php	$postmeta = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $wpdb->postmeta WHERE post_id = %d", $post->ID ) );
		foreach ( $postmeta as $meta ) :
			 ...
			 */
			if ( apply_filters( 'tf_wxr_export_skip_postmeta', false, $meta->meta_key, $meta ) )
				continue;
		?>
		<wp:postmeta>
